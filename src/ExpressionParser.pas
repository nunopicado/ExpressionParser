...
const
  ...
  SFuncNames: array [1 .. MaxSFuncs] of ShortString = ('MEDIA', 'HIPOTENUSA',
    'DIV', 'MOD');
...
begin
  ...
  Expression := Copy(Expression, 1, ParStart) + Expr +
    ...
    case NumSFunc of
      ...
      2:
        begin // Hipotenusa
          Value := Param + ' ';
          Param := 'RAIZ ( QUADRADO ' +
            Copy(Value, 1, pos(' ', Value) - 1) + ' + ';
          Delete(Value, 1, pos(' ', Value));
          Param := Param + 'QUADRADO ' +
            Copy(Value, 1, pos(' ', Value) - 1) + ') ';
          Value := FloatToStr(Parse(Param));
        end;
      3:
...
begin
  ...
  try
    case NumFunc of
      1:
        Value := FloatToStr(Sin(GetNextExpressionValue(Expression, FuncStart)));
      2:
        Value := FloatToStr(Cos(GetNextExpressionValue(Expression, FuncStart)));
      3:
        Value := FloatToStr(Tan(GetNextExpressionValue(Expression, FuncStart)));
      4:
        Value := FloatToStr(Sqrt(GetNextExpressionValue(Expression, FuncStart)));
      5:
        Value := FloatToStr(Sqr(GetNextExpressionValue(Expression, FuncStart)));
      6:
        Value := FloatToStr(Power(GetNextExpressionValue(Expression, FuncStart), 3));
      7:
        Value := IntToStr(Round(GetNextExpressionValue(Expression, FuncStart)));
      8:
        Value := IntToStr(Trunc(GetNextExpressionValue(Expression, FuncStart)));
      9:
        Value := FloatToStr(Abs(GetNextExpressionValue(Expression, FuncStart)));
      10:
        Value := FloatToStr(Frac(GetNextExpressionValue(Expression, FuncStart)));
      11:
        if Odd(Trunc(GetNextExpressionValue(Expression, FuncStart)))
          then Value := '1'
          else Value := '0';
      12:
        Value := FloatToStr(Ln(GetNextExpressionValue(Expression, FuncStart)));
      13:
        Value := FloatToStr(Log2(GetNextExpressionValue(Expression, FuncStart)));
      14:
        Value := FloatToStr(Log10(GetNextExpressionValue(Expression, FuncStart)));
      15:
        Value := FloatToStr(ArcSin(GetNextExpressionValue(Expression, FuncStart)));
      16:
        Value := FloatToStr(ArcCos(GetNextExpressionValue(Expression, FuncStart)));
      17:
        Value := FloatToStr(ArcTan(GetNextExpressionValue(Expression, FuncStart)));
      18:
        Value := FloatToStr(CoTan(GetNextExpressionValue(Expression, FuncStart)));
      19:
        Value := FloatToStr(DegToRad(GetNextExpressionValue(Expression, FuncStart)));
      20:
        Value := FloatToStr(RadToDeg(GetNextExpressionValue(Expression, FuncStart)));
    end;
  except
    Value := '0';
  end;
    ...
    then begin
      ...
      if OpPos > length(Expression)
        then begin
          ...
          Op := Expression[OpPos];            // Guarda qual o operador de prioridade 2 encontrado
        end
...
function FindLastValueStart(Expression: ShortString;
