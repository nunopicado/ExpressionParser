...
const
  ...
  FuncNames: array [1..MaxFuncs] of ShortString = (
    'SEN', 'COS', 'TAN', 'RAIZ', 'QUADRADO', 'CUBO', 'ROUND', 'TRUNC', 'ABS',
...
function TParser.Help(Level: Integer): string;
begin
  ...
  if Level in [0, 1]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): Parse' + BR +
                            'Devolve em formato numérico (Real) o valor da expressão passada por' + BR +
                            'parâmetro!' + BR +
                            'Uso: <real> := Parse(Expressão: ShortString);     Exemplo: r := Parse(''1+2+3'');' + BR + BR;

  if Level in [0, 2]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): ParseF' + BR +
                            'Devolve em formato alfanumérico (ShortString) o valor da expressão passada por' + BR +
                            'parâmetro!' + BR +
                            'Uso: <string> := Parse(Expressão: ShortString; Precisão, Decimais: Integer);' + BR +
                            'Exemplo: s := ParseF(''1+2+3'',3,2);' + BR +
  ...
  if Level in [0, 3]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): History' + BR +
                            ...
                            '- Terminar o Histórico: Uso: HistoryClose; (Nota: É importante terminar o' + BR +
                            'historico no fim do programa se este tiver sido inicializado!)' + BR + BR;

  if Level in [0, 4]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): Parêntesis' + BR +
                            'É suportado o uso de parêntesis curvos, simples () ou encadeados (())' + BR +
                            'Prioridades: São sempre prioritárias as operações contidas pelo grupo de' + BR +
                            'parêntesis mais interno, até às operações fora de parêntesis.' + BR + BR;

  if Level in [0, 5]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): Constantes' + BR +
                            'As constantes são representações alfabéticas de um valor numérico fixo. Podem' + BR +
                            'ser inseridas normalmente nas expressões.' + BR +
                            'Uso: CONSTANTE   ***   Coloca-se o nome da constante em qualquer lugar da' + BR +
                            'expressão' + BR +
                            '- PI: 3.14159265' + BR +
                            '- E: 2.71828182' + BR + BR;
  ...
  if Level in [0, 6]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): Funções Especiais (múltiplos argumentos)' + BR +
                            'As funções especiais recebem múltiplos argumentos. Podem ser inseridas' + BR +
                            'normalmente nas expressões, mas não é possível encadear funções especiais,' + BR +
                            'embora possam ser usadas funções normais dentro das especiais.' + BR +
                            'Uso: FUNÇÃO [arg1 arg2 arg3 ... arg_n]   ***   Coloca-se o nome da função,' + BR +
                            'seguido dos argumentos, separados por espaços e contidos entre parêntesis' + BR +
                            'rectos [].' + BR +
                            '- MEDIA: Calcula a média de todos os argumentos fornecidos.' + BR +
                            '- HIPOTENUSA: Calcula a hipotenusa de um triângulo rectângulo, cujos catetos são' + BR +
                            'os argumentos 1 e 2.' + BR +
  ...
  if Level in [0, 7]
    then Result := Result + 'A J U D A (' + IntToStr(Level) + '): Funções (argumento único)' + BR +
                            'As funções recebem um único argumento. Podem ser inseridas normalmente nas' + BR +
                            'expressões, e podem ser encadeadas desde que usem parêntesis em redor das' + BR +
                            'expressões internas Ex.: ABS (SEN 5) .' + BR +
                            'Uso: FUNÇÃO Arg  ou  FUNÇÃO (Expressão)   ***   Coloca-se o nome da função,' + BR +
                            'seguido do argumento. O Argumento pode ser uma expressão também, se contido' + BR +
                            'entre parêntesis curvos.' + BR +
...
procedure TParser.HistoryClear;
begin
  History := TStringList.Create;
end;

function TParser.HistoryShow: string;
var
  i: Integer;
begin
  Result := '';
  for i := 0 to History.Count-1 do
    begin
      if i > 0
        then Result := Result + BR;
      Result := Result + IntToStr(i + 1) + '. ' + History[i];
    end;
end;

procedure TParser.HistoryClose;
begin
  History.Free;
end;

procedure TParser.HistoryAdd(Expression: ShortString);
begin
  if History <> nil
    then History.Add(Expression);
end;

function TParser.Parse(Expression: ShortString): Real;
begin
...
function TParser.ParseF(Expression: ShortString; Precision, Digits: Integer): ShortString;
begin
  Result := FloatToStrF(
    Parse(Expression),
    ffGeneral,
...
procedure TParser.ParseParentesis(var Expression: ShortString);
var
  Expr     : ShortString;
  i        : Integer;
  ParStart : Integer;
...
begin
  ...
  Expression :=
    Copy(Expression, 1, ParStart)
    + Expr
    + Copy(Expression, ParEnd, 999);
...
procedure TParser.ParseConstants(var Expression: ShortString);
var
  Preffix    : ShortString;
  Suffix     : ShortString;
  Value      : ShortString;
...
begin
  ...
  Suffix := Copy(
    Expression,
    ConstStart + Length(ConstNames[NumConst]) + 1,
    999
  );
...
procedure TParser.ParseSpecialFunctions(var Expression: ShortString);
var
  Preffix    : ShortString;
  Suffix     : ShortString;
  Param      : ShortString;
...
procedure TParser.ParseFunctions(var Expression: ShortString);
var
  Preffix   : ShortString;
  Suffix    : ShortString;
  Value     : ShortString;
...
begin
  ...
  try
    case NumFunc of
      ...
      11: if Odd(Trunc(GetNextExpressionValue(Expression, FuncStart)))
            then Value := '1'
            else Value := '0';
...
procedure TParser.ParseOperators(var Expression: ShortString);
var
  Preffix : ShortString;
  Suffix  : ShortString;
  Value   : ShortString;
...
begin
  ...
  Preffix := Copy(
    Expression,
    1,
    FindLastValueStart(Expression, OpPos) - 1
  );

  // Memoriza a parte da expressão desde o fim do argumento a ser processado até ao fim da expressão
  Suffix := Copy(
    Expression,
    FindNextValueEnd(Expression, OpPos) + 1,
    999
  );
  ...
  try
    case Op of
      '=': if GetLastExpressionValue(Expression, OpPos) = GetNextExpressionValue(Expression, OpPos)
             then Value := '1'
             else Value := '0';
      '!': if GetLastExpressionValue(Expression, OpPos) <> GetNextExpressionValue(Expression, OpPos)
             then Value := '1'
      ...
      '*',
      'x',
      'X': Value := FloatToStr(GetLastExpressionValue(Expression, OpPos) * GetNextExpressionValue(Expression, OpPos));
      '/',
...
function TParser.FindLastValueStart(Expression: ShortString; StartPoint: Integer): Integer;
begin
  // Inicializa o resultado (se nada for feito, o resultado será igual ao ponto de partida
  Result := StartPoint;

  // Encontra a posição do fim do valor
  while (Result >= 1) and (Expression[Result - 1] in ['0'..'9', '-', FormatSettings.DecimalSeparator]) do
    Dec(Result);
end;

function TParser.FindNextValueEnd(Expression: ShortString; StartPoint: Integer): Integer;
begin
  // Inicializa o resultado (se nada for feito, o resultado será igual ao ponto de partida
  Result := StartPoint;

  // Passa à frente, caso haja, caracteres que não pertençam ao valor
  while not (Expression[Result] in ['0'..'9', '-', FormatSettings.DecimalSeparator]) do
    Inc(Result);

  // Encontra a posição do inicio do valor
  while Expression[Result+1] in ['0'..'9','-',FormatSettings.DecimalSeparator] do
    Inc(Result);
end;

function TParser.GetLastExpressionValue(Expression: ShortString; StartPoint: Integer): Real;
var
  Value : ShortString;
  i     : Integer;
begin
  ...
  while not (Expression[Length(Expression)] in ['0'..'9', '-', FormatSettings.DecimalSeparator]) do
    Delete(Expression, Length(Expression), 1);

  // Guarda os caracteres correspondentes ao valor
  Value := '';
  i := Length(Expression);
  while (i >= 1) and (Expression[i] in ['0'..'9', '-', FormatSettings.DecimalSeparator]) do
    begin
...
function TParser.GetNextExpressionValue(Expression: ShortString; StartPoint: Integer): Real;
var
  Value : ShortString;
  i     : Integer;
begin
  ...
  while not (Expression[1] in ['0'..'9', '-', FormatSettings.DecimalSeparator]) do
    Delete(Expression, 1, 1);

  // Guarda os caracteres correspondentes ao valor
  Value := '';
  i := 1;
  while (i <= Length(Expression)) and (Expression[i] in ['0'..'9', '-', FormatSettings.DecimalSeparator]) do
    begin
...
procedure TParser.PrepareParentesis(var Expression: ShortString);
var
  i           : Integer;
  j           : Integer;
  CountOpen   : Integer;
      ...
      '(': if i > 1
             then begin
               ...
               repeat
                 Dec(j);
               until Expression[j] <> ' ';
               if Expression[j] in ['0'..'9', FormatSettings.DecimalSeparator]
                 then pExpr := pExpr + '*('
                 else pExpr := pExpr + '(';
             end
...
procedure TParser.PrepareOpExpression(var Expression: ShortString);
var
  i     : Integer;
  pExpr : ShortString;
begin
      ...
      case Expression[i] of
        ...
        '-': case Expression[i + 1] of
               ...
               '-': begin
                    ...
                    if i > 1
                      then pExpr := pExpr + '+-'     // Se não for o sinal de um 1º valor negativo, converte-se em +-, onde o - passa a pertencer ao valor e o + é a operação
                      else pExpr := pExpr + '-';
                    Inc(i, 1);
                    end;
             end;
...
function TParser.CharCount(ch: AnsiChar; Text: ShortString): Integer;
var
  i: Integer;
begin
  Result:=0;
...
function TParser.wPos(SubStr, Str: ShortString): Integer;
begin
  Result := Pos(SubStr, Str);
  if Result > 1
    then
      if Upcase(Str[Result - 1]) in ['A'..'Z']
        then Result:=0;
end;
